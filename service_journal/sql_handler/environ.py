		...
		'dbt_sql_map': {
			...
			'views_tables': {
				'actual': {
					...
					'static': 'SELECT [service_day],[block],[trip26],[bus],[Time],[Departure_Time],[Route],[dir],[Stop_Id],[Stop_Name],[Boards],[Alights],[Onboard],[OperationalStatus],[Latitude],[Longitude],[Operator_Record_Id] FROM [TA_ITHACA_ACTUAL_HISTORY].[dbo].[v_vehicle_history] WHERE [service_day]=? ORDER BY [service_day],[block],[trip26],[Time] asc',
				...
				'scheduled': {
					'deflt_query': 'SELECT ?,?,?,?,?,?,?,?,?,?,?,?,? FROM ? WHERE ?=?',
					'opt_query': 'SELECT ?,?,?,?,?,?,?,?,?,?,?,?,? FROM ? WHERE ?=? AND ?=?',
					'static':'SELECT [Service_Date], [Direction], [block], [trip26], [stop_num], [next_stop], [departure_time], [route] FROM [TA_ITHACA_SCHEDULE_HISTORY].[dbo].[v_scheduled_stops] WHERE [Service_Date]=? ORDER BY [Service_Date],[BlockNumber],[Trip26],[DepartureTime] asc',
